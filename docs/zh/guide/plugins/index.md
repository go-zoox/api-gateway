...
| [IP 策略](./ip-policy) | `plugin/ippolicy` | 全局 `ip_policy.enable` 为真，或任一路由启用 `ip_policy.enable` |
| [CORS](./cors) | `plugin/cors` | 全局 `cors.enable` 为真，或任一路由启用 `cors.enable` |
| [限流](./rate-limit) | `plugin/ratelimit` | 全局 `rate_limit.enable` 为真，或任一路由启用 `rate_limit.enable` |
| [JSON 审计](./json-audit) | `plugin/jsonaudit` | 顶层 `json_audit.enable` 为真，或任一路由启用 `json_audit.enable` |
...
- [IP 策略](./ip-policy) — CIDR 允许/拒绝；可信反代与 `X-Forwarded-For`。
- [CORS](./cors) — 浏览器跨域与预检响应。
- [限流](./rate-limit) — 令牌桶、漏桶、固定窗口；内存或 Redis。
- [JSON 审计](./json-audit) — 当响应为 JSON 类时记录请求与响应体（脱敏）供审计。
